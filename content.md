...
<br><br>
The table reports label accuracy (correct Accept/Reject) and hack success (a generated input that legally breaks a wrong candidate), all single-sample. Training does the heavy lifting: the same 35B backbone scores **16.2** overall hack success untrained, **40.8** with an agentic scaffold, and **63.7** after RL. That puts Arbiter **11.6 points above DeepSeek-V4-Pro, a model 45× its size**, and ahead of every open-weight model except GLM-5.2. The strongest closed models, GPT-5.5 and Claude-Opus-4.8, remain ahead; we do not claim otherwise.
