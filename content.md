...
## refinement-cap
**One round of refinement does most of the work.** The line is contest score when Arbiter-gated refinement is capped at round **R**: it jumps **272.49 → 356.91** from a single round of Arbiter feedback — most of the eventual gain, though not quite gold yet — then reaches **385.46** by R4, clearing gold, and holds there through R5. The bars show how many trajectories are still being sampled each round per subtask — all **50** seeds at R0, dropping to **20** at R1 and just **7** by R5 as accepted solutions leave the pool. Most of the score is banked while the pool is still large.
